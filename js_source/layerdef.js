...
// ====================
// The query contains strings for the various layers
//
// make_layer format variables:
//
//<URL> string 'url',<color>: string '#RGB', <name>: string '[imagetype]text', <linewidth>[.circleradius]: int/float, <visibility> : boolean, [linetype][transparency] :string '[on off (on off ( ...))][@transparency]'
//
// imagetype: #l# = line, #dl#=double line, #d# = dotted line, #c#= transparent, #co# = opaque circle with digit
// on/off = lenght of the line (in pixel), visible line-open portion
// transparency = 0-1 transparencye of the line
//
//=====================
	function layerdef(type){

		if (type == "oneway"){
		//	This makes the layers for the oneway tags
		map.addLayers([
			// characteristics of the oneway

			// boundary=administrative and admin_level=4 or 8 - black line
			make_layer(QURL + "?data=(way[boundary=administrative][admin_level=4](bbox);node(w);way[boundary=administrative][admin_level=8](bbox);node(w););out+skel;", "black",name="#l#boundary=administrative, admin_level=4 or 8", 6, true),

			// boundary=administrative and admin_level=9 - dotted black line
			make_layer(QURL + "?data=(way[boundary=administrative][admin_level=9](bbox);node(w);way[boundary=administrative][admin_level=10](bbox);node(w););out+skel;", "black",name="#dl#boundary=administrative, admin_level=9", 4, false,"5 10"),

			// oneway street - red line
			make_layer(QURL + "?data=(way[highway~'^unclas|^living|^resid|road|cycleway'][oneway~'yes|true|1|-1'][cycleway!~'.'][bicycle!=no]['bicycle:oneway'!=no]['oneway:bicycle'!=no](bbox);node(w);way['bicycle:oneway'~'yes|true|1|-1'](bbox);node(w);way['oneway:bicycle'~'yes|true|1|-1'](bbox);node(w););out+skel;", "red",name="#l#oneway street", 4, true),

			// cycleway=opposite or oneway:bicyle=no - green line
			make_layer(QURL + "?data=(way['oneway:bicycle'=no](bbox);node(w);way[cycleway~'opposite'](bbox);node(w);way['bicycle:oneway'= no](bbox);node(w););out+skel;", "green",name="#l#cycleway=opposite or<br>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsponeway:bicyle=no", 4, true)
		]);
	}
	...
	map.addLayers([
		...
		// highway = motorway - red line
		make_layer(QURL + "?data=(way['note'~'autoroute'](bbox);node(w););out+skel;", "purple",name="#l#autoroute", 6, true),

		// highway = secondary - red line
		make_layer(QURL + "?data=(way[highway=secondary](bbox);node(w););out+skel;", "red",name="#l#voie de transit regional", 4, true),

		// highway = tertiary - orange line
		make_layer(QURL + "?data=(way[highway=tertiary](bbox);node(w););out+skel;", "orange",name="#l#voie de transit sous-regional", 4, true)

		// highway = unclassified - blue line
		//make_layer(QURL + "?data=(way[highway=unclassified](bbox);node(w););out+skel;", "blue",name="#l#voie de liaison", 4, true),

		// highway = tertiary - orange line
		//make_layer(QURL + "?data=(way[highway=tertiary](bbox);node(w););out+skel;", "green",name="#l#voie de distribution", 4, true)
...
map.addLayers([
	...
	// boundary=administrative and admin_level=4 or 8 - gray line
	make_layer(QURL + "?data=(way[boundary=administrative][admin_level=4](bbox);node(w);way[boundary=administrative][admin_level=8](bbox);node(w););out+skel;", "gray",name="#l#boundary=administrative, admin_level=4 or 8", 6, true),

	// boundary=administrative and admin_level=9 - dotted gray line
	make_layer(QURL + "?data=(way[boundary=administrative][admin_level=9](bbox);node(w);way[boundary=administrative][admin_level=10](bbox);node(w););out+skel;", "gray",name="#dl#boundary=administrative, admin_level=9", 4, false,"5 10"),

	// railway = rail - black line
	make_layer(QURL + "?data=(way[railway=rail](bbox);node(w););out+skel;", "black",name="#l#railway=rail", 4, true),

	// railway = railway_crossing - black line
	make_layer(QURL + "?data=(way[railway=railway_crossing](bbox);node(w););out+skel;", "black",name="#dl#railway=rail", 4, true, "5 10"),

	// waterway = stream - blue line
	make_layer(QURL + "?data=(way[waterway=stream](bbox);node(w););out+skel;", "blue",name="#l#waterway=stream", 4, true),

	// natural = water - blue line
	make_layer(QURL + "?data=(way[natural=water](bbox);node(w););out+skel;", "blue",name="#dl#natural=water", 4, true, "5 10")

]);
}

		if (type == "cycleways"){
		//	This makes the layers for the cycleway tags
			map.addLayers([
			// highway=cycleway - Red line
      ...
      make_layer(QURL + "?data=(way[highway=cycleway][mofa=no](bbox);node(w););out+skel;", "#00FFFF",name="#l#cycleway mofa=no", 3, false),
			...
			// cycleway=lane - dotted orange line
      make_layer(QURL + "?data=(way[cycleway=lane](bbox);node(w);way[cycleway=opposite_lane](bbox);node(w);way['cycleway:right'=opposite_lane](bbox);node(w);way['cycleway:left'=opposite_lane](bbox);node(w);way['cycleway:left'=lane](bbox);node(w););out+skel;","#ff6541",name="#dl#cycleway=lane", 6, false,"6 10@0.9"),
			...
			// cycleway=shared_lane - dotted red line
			...
			// characteristics of the oneway
			// oneway street - dotted blue line
			...
			// cycleway=opposite or oneway:bicyle=no - dotted green line
      make_layer(QURL + "?data=(way['oneway:bicycle'=no](bbox);node(w);way[cycleway~'opposite'](bbox);node(w);way['bicycle:oneway'= no](bbox);node(w););out+skel;", "green",name="#dl#cycleway=opposite or<br>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsponeway:bicyle=no", 3, false,"6 10"),
			...
			// bicycle=yes & footway, path, track, steps - green line
      ...
      make_layer(QURL + "?data=(way[bicycle=use_sidepath](bbox);node(w););out+skel;","#bd65d5",name="#dl#bicycle=use_sidepath", 4, false,"6 10"),
		...
		if (type == "surface"){
		//	This makes the layers for the surface layer
			map.addLayers([
	  		//highways
		...
		if (type == "route"){
			...
			// Official LF routes from the Fietsplatform
	...
	if (type == "bugs"){
		//	This makes the layers for the Bug layer
			map.addLayers([
